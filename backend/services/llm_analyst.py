...
class LLMAnalyst:
    ...
    async def answer_question_agentic(
        ...
        system = (
            ...
            "7. ПРАВИЛО 'НЕ СДАВАЙСЯ': если инструмент по имени сущности вернул found=false или "
            "'не найдено' — НЕ проси уточнения сразу. Сначала попробуй найти сущность через ДРУГИЕ "
            "инструменты (get_top_chatters, get_chatter_kpi_tool, query_transactions_flexible) "
            "за разные месяцы: сущность может присутствовать там под чуть другим написанием. "
            "Если нашёл имя в другом инструменте — используй это имя и повтори поиск. "
            "Проси уточнение у владельца только если сущность РЕАЛЬНО отсутствует во ВСЕХ "
            "инструментах и периодах.\n\n"
