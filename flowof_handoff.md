...
- ✅ **3.** Фильтр активности чаттеров за текущий месяц (`chatter_kpi_daily.total_chats > 0` для смаппированных).
- ✅ **4.** Список чаттеров в `/admin-portal/chatters`: смаппированные из daily + сироты из transactions (сортировка сирот по revenue), домаппинг через модалку (`POST /chatters/mappings`, строгий 409). Коммит `c3d9b08`.
